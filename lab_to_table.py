...
def is_skip_line(line: str) -> bool:
    s = line.strip()
    return (not s or s in SKIP_EXACT or SECTION_HEADER_RE.match(s) is not None or COLUMN_HEADER_RE.match(s) is not None or
            COMMENT_LINE_RE.match(s) is not None or MARKDOWN_HEADER_RE.match(s) is not None or MARKDOWN_RULE_RE.match(s) is not None or
            META_LINE_RE.search(s) is not None or s.startswith("- "))
...
def is_qual_skip_line(line: str) -> bool:
    s = line.strip()
    return not s or s in QUAL_SKIP_EXACT or COLUMN_HEADER_RE.match(s) is not None or COMMENT_LINE_RE.match(s) is not None or META_LINE_RE.search(s) is not None
...
def compose_value(value: str, flag: str = "") -> str:
    return f"{value.strip()} {flag.strip()}".strip()


def value_flag(value: str) -> str:
    return "up" if str(value).endswith("▲") else "down" if str(value).endswith("▼") else ""
...
def classify_qual_status(result: str, ref: str = "") -> str:
    r, refn = norm_text(result), norm_text(ref)
    if any(w in refn for w in NORMAL_WORDS):
        if any(w in r for w in ABNORMAL_WORDS):
            return "abnormal"
        if any(w in r for w in NORMAL_WORDS):
            return "normal"
    rm = re.search(r"(-?\d+(?:\.\d+)?)\s*~\s*(-?\d+(?:\.\d+)?)", r)
    refm = re.search(r"(-?\d+(?:\.\d+)?)\s*~\s*(-?\d+(?:\.\d+)?)", refn)
    if rm and refm:
        ...
        if r_low > ref_high:
            return "high"
        return "normal"
    m = re.search(r"(-?\d+(?:\.\d+)?)\s*이상", r)
    if m and refm and float(m.group(1)) > float(refm.group(2)):
        return "high"
    m = re.search(r"(-?\d+(?:\.\d+)?)\s*이하", r)
    if m and refm and float(m.group(1)) < float(refm.group(1)):
        return "low"
    if refn.startswith("<"):
        rv, xv = _first_num(r), _first_num(refn)
        if rv is not None and xv is not None:
...
def parse_candidate_row(line: str, section: str = "", draw_time: str = "") -> Optional[LabRow]:
    ...
    return LabRow(name=name, value=compose_value(value, flag), unit=re.sub(r"\s+", " ", unit).strip(), ref=re.sub(r"\s+", " ", ref).strip(), raw_line=line, section=section, draw_time=draw_time, table_title=format_title(section or "Unknown", draw_time))
...
def parse_qualitative_row(line: str, section: str = "", draw_time: str = "") -> Optional[QualRow]:
    ...
    if classify_row_type(result, ref, "") != "qualitative":
        return None
    return QualRow(item=item, result=re.sub(r"\s+", " ", result).strip(), ref=re.sub(r"\s+", " ", ref).strip(), status=classify_qual_status(result, ref), note=re.sub(r"\s+", " ", note).strip(), raw_line=line, section=section, draw_time=draw_time, table_title=format_title(section or "Unknown", draw_time))
...
def parse_lab_text(text: str) -> Tuple[List[LabRow], List[QualRow], List[str], List[str]]:
    ...
    for raw in text.splitlines():
        ...
        if is_qualitative_section(current_section):
            ...
            if qrow:
                qual_rows.append(qrow); last_qual_row = qrow; last_row = None; continue
        ...
        if row:
            ...
            if row_type == "qualitative":
                qual_rows.append(QualRow(item=row.name, result=row.value, ref=row.ref, status=classify_qual_status(row.value, row.ref), raw_line=row.raw_line, section=row.section, draw_time=row.draw_time, table_title=row.table_title))
                last_qual_row = qual_rows[-1]; last_row = None
            else:
                rows.append(row); last_row = row; last_qual_row = None
            continue
...
def rows_to_dataframe(rows: List[LabRow]):
    import pandas as pd
    return pd.DataFrame([{"제목": r.table_title, "검사명": r.name, "결과값": r.value, "단위": r.unit, "참고치": r.ref, "검사종류": r.section, "채혈시각": r.draw_time} for r in rows])


def qual_rows_to_dataframe(rows: List[QualRow]):
    import pandas as pd
    return pd.DataFrame([{"제목": r.table_title, "항목": r.item, "결과": r.result, "참고치": r.ref, "판정": r.status, "비고": r.note, "검사종류": r.section, "채혈시각": r.draw_time} for r in rows])


def report_lines_to_dataframe(lines: List[str]):
    import pandas as pd
...
def style_lab_df(df):
    def style_row(row):
        color = ""
        if value_flag(str(row.get("결과값", ""))) == "up":
            color = "color: #C62828;"
        elif value_flag(str(row.get("결과값", ""))) == "down":
            color = "color: #1565C0;"
        return [color] * len(row)
    return df.style.apply(style_row, axis=1) if not df.empty else df


def style_qual_df(df):
    def style_row(row):
        status = str(row.get("판정", ""))
        color = "color: #C62828;" if status in ("high", "abnormal") else "color: #1565C0;" if status == "low" else ""
        return [color] * len(row)
    return df.style.apply(style_row, axis=1) if not df.empty else df
...
def build_excel_bytes(rows: List[LabRow], qual_rows: List[QualRow], raw_text: str, unparsed_lines: List[str], report_lines: List[str]) -> BytesIO:
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for title, group_rows in grouped.items():
            ...
            c = ws_group.cell(row=cur, column=1, value=title); c.font = title_font; c.fill = title_fill; c.alignment = Alignment(horizontal="left")
            cur += 1
            for i, h in enumerate(["검사명", "결과값", "단위", "참고치"], start=1):
                cc = ws_group.cell(row=cur, column=i, value=h); cc.font = header_font; cc.fill = header_fill
            cur += 1
            for r in group_rows:
                cells = [ws_group.cell(row=cur, column=1, value=r.name), ws_group.cell(row=cur, column=2, value=r.value), ws_group.cell(row=cur, column=3, value=r.unit), ws_group.cell(row=cur, column=4, value=r.ref)]
                vf = value_flag(r.value)
                if vf == "up":
                    [setattr(cell, 'font', red_font) for cell in cells]
                elif vf == "down":
                    [setattr(cell, 'font', blue_font) for cell in cells]
                cur += 1
            cur += 2
        ...
        for title, group_rows in qual_grouped.items():
            ...
            c = ws_qgroup.cell(row=cur, column=1, value=title); c.font = title_font; c.fill = title_fill; c.alignment = Alignment(horizontal="left")
            cur += 1
            for i, h in enumerate(["항목", "결과", "참고치", "판정", "비고"], start=1):
                cc = ws_qgroup.cell(row=cur, column=i, value=h); cc.font = header_font; cc.fill = header_fill
            cur += 1
            for r in group_rows:
                cells = [ws_qgroup.cell(row=cur, column=1, value=r.item), ws_qgroup.cell(row=cur, column=2, value=r.result), ws_qgroup.cell(row=cur, column=3, value=r.ref), ws_qgroup.cell(row=cur, column=4, value=r.status), ws_qgroup.cell(row=cur, column=5, value=r.note)]
        ...
        for sheet_name, df in {"Parsed_Labs": parsed_df, "Qualitative_Results": qual_df, "Original_Text": raw_df, "Unparsed": unparsed_df, "Ignored_Reports": report_df}.items():
            ...
            if sheet_name == "Parsed_Labs" and not df.empty:
                for row_idx in range(2, len(df) + 2):
                    vf = value_flag(str(ws.cell(row=row_idx, column=3).value or ""))
                    if vf:
...
def render_lab_to_excel_tool() -> None:
    ...
    st.info("보고서형 검사(영상/내시경/병리)는 표 변환 대신 Text Cleaner 사용을 권장합니다. 정량은 기존 방식대로, 정성은 결과/참고치 비교로 판정합니다.")
    ...
    raw_text = st.text_area("Lab / EMR 원문 붙여넣기", key="lab_excel_input", height=360, placeholder="여기에 검사 결과 원문을 붙여넣으세요...")
    c1, c2 = st.columns(2)
    run = c1.button("표 만들기", type="primary", use_container_width=True)
    clear = c2.button("입력 지우기", use_container_width=True)
    if clear:
        st.session_state["lab_excel_input"] = ""; st.session_state["lab_results"] = None; st.rerun()
    if run:
    ...
    if st.session_state.get("lab_results") is not None:
        ...
        cols = st.columns(6)
        cols[0].metric("정량 row", len(rows)); cols[1].metric("정성 row", len(qual_rows)); cols[2].metric("정량 표 수", len(grouped)); cols[3].metric("미파싱 줄", len(unparsed_lines)); cols[4].metric("보고서 줄", len(report_lines)); cols[5].metric("원문 줄 수", len(raw_text.splitlines()))
        now_str = datetime.now(timezone(timedelta(hours=9))).strftime("%Y%m%d_%H%M")
        c1, c2 = st.columns(2)
        c1.download_button("📥 엑셀 다운로드", data=res["excel_bytes"], file_name=f"table_{now_str}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        c2.download_button("📥 TSV 다운로드", data=res["tsv_text"], file_name=f"table_{now_str}.tsv", mime="text/tab-separated-values", use_container_width=True)
        st.markdown("### 정량 결과 미리보기")
        for title, group_rows in grouped.items():
            st.markdown(f"#### {title}")
            st.dataframe(style_lab_df(rows_to_dataframe(group_rows)[["검사명", "결과값", "단위", "참고치"]]), use_container_width=True, hide_index=True)
            st.write("")
        if qual_grouped:
            st.markdown("### 정성 결과 미리보기")
            for title, group_rows in qual_grouped.items():
                st.markdown(f"#### {title}")
                st.dataframe(style_qual_df(qual_rows_to_dataframe(group_rows)[["항목", "결과", "참고치", "판정", "비고"]]), use_container_width=True, hide_index=True)
                st.write("")
        ...
        if report_lines:
            st.info("보고서형 검사(영상/내시경/병리 등)는 Text Cleaner 사용을 권장합니다.")
